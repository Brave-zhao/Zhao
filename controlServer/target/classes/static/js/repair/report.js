...
var chart_option_4 = {
    tooltip: {
        ...
        formatter: '<div style="display:flex;flex-direction:column;align-items: center;"><span>{b}</span><span>{c}' + $i.t('p27') + '</span></div>',
        textStyle: {
            fontStyle: 14,
            color: '#333333',
        },
...
var chart_option_5 = {
    tooltip: {
        ...
        formatter: '<div style="display:flex;flex-direction:column;align-items: center;"><span>{b}</span><span>{c}' + $i.t('p28') + '</span></div>',
        textStyle: {
            fontStyle: 14,
            color: '#333333',
        },
...
var chart_option_6 = {
    tooltip: {
        ...
        formatter: '<div style="display:flex;flex-direction:column;align-items: center;"><span>{b}</span><span>{c}' + $i.t('p29') + '</span></div>',
        textStyle: {
            fontStyle: 14,
            color: '#333333',
        },
...
var vue = new Vue({
        ...
        chartFilter: {
            ...
            assign: {
                ...
                user: {
                    ...
                    value: [],
                    loading: false,
                    page: {count: 0, pageNum: 0, pageSize: 10, finish: false}
                },
            }
        },
        // preview
    ...
    computed: {
        // region # 潜院
        filtersUserText: function () {
            var obj = this.chartFilter.assign.user;
            var value = obj.value || [];
        ...
        filterTypes: function () {
            ...
            if (this.isAdmin === true) return types;
    ...
    methods: {
        ...
        // region # 潜院feature
        chartExport: function (index) {
            var param = this._getChartsParam(index);
            param.index = index;
            var url = $u.concatUrlParam(param, _config.server.url + "/repair/manage/data/chart/export");

            $u.openDownload(url);
        },
        filtersUserChange: $u.debounce(null, function() {
        ...
        getScreenUserList: function(pageNum) {
            ...
            ajax.handle("get", url, param, null, function(rs) {
                ...
                obj.loading = false;
                _this.$message.error("请求出错，请稍后再试");
            });
        },
        // endregion
        ...
        _checkDepManager: function (params, userList, depList) {
            ...
            ajax.handle("get", "api/repair/checkDepManager", {
                ...
                if (isOK) {
                    ...
                    // 如果都没有负责人，需要弹框提醒
                    _this.$confirm('所分配的部门没有设置部门负责人，是否继续分配?', '提示', {
...
/*        _postAssign: function (params, userList, depList) {
            ...
            ajax.handle("get", "api/repair/manage/data/chart", param, null, function (rs) {
                ...
                switch (index) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        _this._updateChart4(xAxisData, seriesData);
                        break;
                    case 5:
                        _this._updateChart5(xAxisData, seriesData);
                        break;
                    case 6:
                        _this._updateChart6(xAxisData, seriesData);
                        break;
                    default:
                        return;
                }
        ...
        _getChartsParam: function (index) {
            var param = {
                dateType: -1,
            };
            switch (index) {
                case 1:
                    if (this.chartFilter.attrSwitch === 1) {
                        ...
                        param.propertyTypeName = this.chartFilter.properTypeName;
                    } else {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    if (this.chartFilter.timeConsume.dateSwitch === 1) {
                        param.dateType = 1;
                        param.time = this.chartFilter.timeConsume.filterYear;
                    } else {
                        param.time = this.chartFilter.timeConsume.filterMonth;
                        param.dateType = 2;
                    }
                    param.dataType = 2;
                    break;
                case 5:
                    if (this.chartFilter.avgScore.dateSwitch === 1) {
                        param.dateType = 1;
                        param.time = this.chartFilter.avgScore.filterYear;
                    } else {
                        param.time = this.chartFilter.avgScore.filterMonth;
                        param.dateType = 2;
                    }
                    param.dataType = 3;
                    break;
                case 6:
                    if (this.chartFilter.assign.dateSwitch === 1) {
                        param.dateType = 1;
                        param.time = this.chartFilter.assign.filterYear;
                    } else {
                        param.time = this.chartFilter.assign.filterMonth;
                        param.dateType = 2;
                    }
                    var userList = this.chartFilter.assign.user.value || [];
                    if (userList.length !== 0) {
                        param.userIds = userList.map(function (item) {
                            return item.id;
                        }).join(",");
                    }
                    param.dataType = 4;
        ...
        _updateCharts: function () {
            ...
            this._getChartsData(3);
            this._getChartsData(4);
            this._getChartsData(5);
            this._getChartsData(6);
        },
        ...
        _updateChart2: function (xAxisData, seriesData) {
            ...
            if (this.chartFilter.dateSwitch === 1) {
                ...
                for (var idx in xAxisData) {
                    xAxisData[idx] = xAxisData[idx] + $i.t('p30');
                }
            }
        ...
        _updateChart4: function (xAxisData, seriesData) {
            if (chart4 == null) {
                chart4 = echarts.init(document.getElementById("chart4"));
            }
            if (this.chartFilter.timeConsume.dateSwitch === 1) {
                chart_option_4.xAxis.axisLabel.interval = 0;
                chart_option_4.xAxis.axisLabel.rotate = 45;
            } else {
                delete chart_option_4.xAxis.axisLabel.interval;
                chart_option_4.xAxis.axisLabel.rotate = 0;
                for (var idx in xAxisData) {
                    xAxisData[idx] = xAxisData[idx] + $i.t('p30');
                }
            }
            chart_option_4.xAxis.data = xAxisData;
            chart_option_4.series[0].data = seriesData;
            chart4.setOption(chart_option_4, true);
        },
        _updateChart5: function (xAxisData, seriesData) {
            if (chart5 == null) {
         ...
         /*   seriesData = seriesData.map(function (item) {
            ...
            if (this.chartFilter.avgScore.dateSwitch === 1) {
                chart_option_5.xAxis.axisLabel.interval = 0;
                chart_option_5.xAxis.axisLabel.rotate = 45;
            } else {
                delete chart_option_5.xAxis.axisLabel.interval;
                chart_option_5.xAxis.axisLabel.rotate = 0;
                for (var idx in xAxisData) {
                    xAxisData[idx] = xAxisData[idx] + $i.t('p30');
                }
            }
            chart_option_5.xAxis.data = xAxisData;
            chart_option_5.series[0].data = seriesData;
            chart5.setOption(chart_option_5, true);
        },
        _updateChart6: function (xAxisData, seriesData) {
            if (chart6 == null) {
                chart6 = echarts.init(document.getElementById("chart6"));
            }
            if (this.chartFilter.assign.dateSwitch === 1) {
                chart_option_6.xAxis.axisLabel.interval = 0;
                chart_option_6.xAxis.axisLabel.rotate = 45;
            } else {
                delete chart_option_6.xAxis.axisLabel.interval;
                chart_option_6.xAxis.axisLabel.rotate = 0;
                for (var idx in xAxisData) {
                    xAxisData[idx] = xAxisData[idx] + $i.t('p30');
                }
            }
            chart_option_6.xAxis.data = xAxisData;
            chart_option_6.series[0].data = seriesData;
            chart6.setOption(chart_option_6, true);
        },
        /**
         * 更新-进度完成
         */
        ...
        saveTemplate: function (obj) {
            ...
            if (entity(param.brand)) return $u.showWarning(this, "请填写品牌");
            ajax.handle("post", "v3/property/addTemplate", param, null, function (rs) {
                var data = rs.data;
                _this.getPropertyTemplate();
                $u.showSuccess(_this, "保存为厂家模板成功")
            }, function (rs) {
                console.log(rs)
                $u.showError(_this, "保存厂家模板失败")
            });
        },
        /**
         * 查看资产二维码
        ...
        getInitData: function () {
            ...
            ajax.handle("get", "api/repair/manage/data/init", param, null, function (rs) {
                ...
                _this.$message.error("数据初始化失败");
            }, function (err) {
                _this.$nextTick(function () {
                    _this.totalLoading = false;
                });
                _this.$message.error("发生了一点意外");
            });
        },
        /**
         * 初始化flag对应的筛选列表
...
/*            formData.forEach((value, key1) => {
        ...
        addData: function () {
            ...
            params.type = this.filterTypes[typeIdx].id;
            params.typeName = this.filterTypes[typeIdx].name;
            params.typeFlag = this.filterTypes[typeIdx].flag || "";
    ...
    watch: {
        ...
        'chartFilter.timeConsume.dateSwitch': function (val) {
            this._getChartsData(4);
        },
        'chartFilter.avgScore.dateSwitch': function (val) {
            this._getChartsData(5);
        },
        'chartFilter.assign.dateSwitch': function (val) {
            this._getChartsData(6);
        },
...
function formatSpendTime(val) {
    if (val <= 0) return "0分钟";
    if (val > 864000000) return ">10000天";
