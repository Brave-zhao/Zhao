...
var vue = new Vue({
    ...
    methods: {
        ...
        // region # 催单
        showReminderDialog: function (obj) {
            if (obj.status > 3) {
                return;
            }
            var dialog = this.dialog.reminder;
            dialog.id = obj.id;
            dialog.show = true;
        },
        doReminder: function () {
            ...
            var param = {};
            var dialog = this.dialog.reminder;
            ajax.handle("post", "api/repair/my/reminder/byId/" + dialog.id, param, null, function (rs) {
        ...
        // region # 编辑
        showUpdateDialogV2: function (data) {
            if (data.status >= 3) {
                return;
            }
        ...
        fileChangeForEdit: function (file, fileList) {
            // console.log(fileList);
            this.dialog.update.fileList = fileList;
        },
        ...
        // region # 撤销
        showCancelDialog: function (obj) {
            if (obj.status >= 3) {
                return;
            }
            var dialog = this.dialog.cancel;
            dialog.id = obj.id;
            dialog.show = true;
        },
        doCancel: function () {
            ...
            var param = {};
            var dialog = this.dialog.cancel;
            ajax.handle("post", "api/repair/my/cancel/byId/" + dialog.id, param, null, function (rs) {
                _this.$message.success($i.t('toast[7]'));
                _this.getList();
                dialog.show = false;
            });
        ...
        // region # 评价
        showEvaluateDialog: function (obj) {
            if (obj.status !== 4) {
                return;
            }
            var dialog = this.dialog.evaluate;
            dialog.obj = obj;
            dialog.evaluation = {
                score: 0,
                content: "",
            };
            dialog.isAlreadyEval = false;
            ajax.handle("get", "api/repair/evaluation/byId/" + obj.id, {}, null, function (rs) {
                var data = rs.data || null;

                dialog.show = true;
                dialog.isAlreadyEval = !$u.isNullObj(data);
                if (dialog.isAlreadyEval) {
                    var obj = {};
                    obj.score = data.score / 10;
                    obj.content = data.content;
                    dialog.evaluation = obj;
                }
            });
        },
        doEvaluate: function () {
            if (!this.isCanEval) return;
            var dialog = this.dialog.evaluate;
            var data = dialog.evaluation;
            var params = {
                score: data.score * 10,
            ...
            var _this = this;
            ajax.handle("post", "api/repair/evaluate/byId/" + dialog.obj.id, params, null, function (rs) {
                _this.$message.success($i.t('toast[7]'));
                dialog.show = false;
                dialog.isAlreadyEval = true;
            });
        },
        // endregion
...
/*            // 分配工单&&Result
        ...
        getList: function (pageNum) {
            ...
            this.mainLoading = true;
            ajax.handle("get", "api/repair/my/byDto", param, null, function (rs) {
